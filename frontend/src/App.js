    ...
    if (edit) {
      ...
      api.post('employes/', data)
        ...
        .catch(err => {
          ...
          // 1. Si on se fait jeter pour un problème d'authentification (401 ou 403)
          if (err.response?.status === 401 || err.response?.status === 403) {
            setMsg({ t: 'error', txt: 'Action non autorisée. Veuillez vous connecter.' });
          } 
          // 2. Si Django renvoie des erreurs de validation (Statut 400 - ex: Matricule ou Téléphone)
          else if (err.response?.data) {
            const erreurs = err.response.data;
            
            // Si le matricule existe déjà, Django renvoie : { matricule: ["Ce champ doit être unique."] }
            if (erreurs.matricule) {
              setMsg({ t: 'error', txt: 'Erreur : Ce matricule existe déjà dans la base.' });
            } else if (erreurs.telephone) {
              setMsg({ t: 'error', txt: 'Erreur : Format de numéro de téléphone invalide.' });
            } else {
              // Optionnel : Regroupe les autres erreurs s'il y en a plusieurs d'un coup
              const detailErreur = Object.entries(erreurs)
                .map(([champ, messages]) => `${champ}: ${messages.join(' ')}`)
                .join(', ');
              setMsg({ t: 'error', txt: `Erreur de saisie : ${detailErreur}` });
            }
          } 
          // 3. Si le serveur ne répond pas du tout
          else {
            setMsg({ t: 'error', txt: 'Erreur lors de la création (serveur injoignable).' });
          }
        });
    }
  };
    ...
    <div className="min-h-screen bg-gray-100 p-6">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-3xl font-bold text-blue-600">Annuaire</h1>
          {token ? (
